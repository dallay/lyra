...
<template>
	<div class="relative inline-flex" @mouseleave="flux.onMouseleave">
		...
		>
			<div v-if="flux.status" ref="tip" class="z-101 fixed left-0 top-0" tabindex="-1">
				<div class="text-pretty rounded bg-slate-500 px-3 py-1 text-sm normal-case text-slate-100">
					{{ title }}
				</div>
			</div>
		</Transition>
