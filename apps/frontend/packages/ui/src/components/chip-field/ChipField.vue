	...
	<FormControl v-slot="{ uid }" :label="label" :required="required" :invalid="invalid" :help="help">
		...
		>
			...
			<input
				...
				v-model="flux.text"
				v-bind="$attrs"
				class="w-fit bg-inherit outline-none"
...
.ChipField {
	@apply flex w-full flex-wrap gap-1 rounded border border-slate-400 bg-white px-3 leading-tight dark:bg-slate-800;

	&.focused {
		@apply ring-primary-500/50 border-primary-400 ring-2;
	}
