	...
	<div class="relative inline-flex text-left" @mouseleave="flux.onMouseleave">
		...
		<Fade>
			<div
				v-if="flux.status"
				ref="dropdown"
				class="z-101 fixed min-w-max rounded-lg border border-gray-200 bg-white shadow-lg dark:border-gray-700 dark:bg-slate-800"
				:class="{
					...
					<slot name="options">
						<div class="px-1 py-2 text-sm">
							<template v-for="option in options" :key="option">
								...
								<div v-else class="my-2 border dark:border-slate-600"></div>
							</template>
						</div>
					</slot>
				</div>
