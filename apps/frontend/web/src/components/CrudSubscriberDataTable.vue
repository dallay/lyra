...
const loadMore = async (pagination: { cursor: string; perPage: number } = {
  cursor: cursor.value,
...
async function refreshData(
  ...
  if (loadMore) {
    const newSubscribers = pageResponse.data.filter(
      (newSubscriber) => !subscribers.value.some(
        (existingSubscriber) => existingSubscriber.id === newSubscriber.id,
      ),
    );
