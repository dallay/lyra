...
<template>
	<div>
		<Popover v-model="status" class="w-full">
			...
			<template #content>
				<div class="p-4">
					<div class="mb-2 text-sm font-semibold">Your password must contain:</div>

					<ul class="space-y-1 text-sm">
						<li class="flex items-center gap-1">
							<div class="h-5 w-5" :class="[min8Chars.test(valueModel) ? passed : failed]"></div>
							Minimum number of characters is 10.
						</li>
						<li class="flex items-center gap-1">
							<div class="h-5 w-5" :class="[lowercase.test(valueModel) ? passed : failed]"></div>
							Should contain lowercase.
						</li>
						<li class="flex items-center gap-1">
							<div class="h-5 w-5" :class="[uppercase.test(valueModel) ? passed : failed]"></div>
							Should contain uppercase.
						</li>
						<li class="flex items-center gap-1">
							<div class="h-5 w-5" :class="[numbers.test(valueModel) ? passed : failed]"></div>
							Should contain numbers.
						</li>
						<li class="flex items-center gap-1">
							<div class="h-5 w-5" :class="[symbols.test(valueModel) ? passed : failed]"></div>
