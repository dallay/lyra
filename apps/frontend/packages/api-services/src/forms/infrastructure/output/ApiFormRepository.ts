...
export default class ApiFormRepository
  ...
  async find(id: string): Promise<FormResponse> {
    ...
    const response = await this.httpRequest.request<FormResponse>(`/forms/${id}`, {
      method: 'GET',
  ...
  async search(
    ...
    const response = await this.httpRequest.request<PageResponse<FormResponse>>('/forms', {
      method: 'GET',
      headers: this.headers,
      params: params,
    });
  ...
  async update(form: Form): Promise<void> {
    const response = await this.httpRequest.request<FormResponse>(`/forms/update/${form.id.value}`, {
      method: 'PUT',
      headers: this.headers,
  ...
  async delete(id: string): Promise<void> {
    const response = await this.httpRequest.request(`/forms/${id}`, {
      method: 'DELETE',
      headers: this.headers,
    });
    if (!response || !response.ok) {
  ...
  async create(form: Form): Promise<void> {
    const response = await this.httpRequest.request<FormResponse>(`/forms/${form.id.value}`, {
      method: 'PUT',
      headers: this.headers,
      body: JSON.stringify(form.toPrimitives()),
    });
