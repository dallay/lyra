...
function sortProperties(aValue: string | number | Date | null | undefined, bValue: string | number | Date | null | undefined) {
  ...
  if (Date.parse(aValue as string) && Date.parse(bValue as string)) {
    return sortDesc.value ? new Date(bValue as string).getTime() - new Date(aValue as string).getTime() : new Date(aValue as string).getTime() - new Date(bValue as string).getTime();
  }
...
const onColumnHeaderClick = (column: ColumnInfo) => {
  if (column.sortable) {
    ...
    const sortedEvent: SortedEvent = { sortKey: sortKey.value, sortType: sortDesc.value ? SortType.DESC : SortType.ASC };
